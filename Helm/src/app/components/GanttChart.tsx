...
export function GanttChart({ initiatives }: GanttChartProps) {
  const { months, startDate, endDate } = useMemo(() => {
    const allDates = initiatives.flatMap(i => [i.startDate, i.endDate]);
  ...
  function getBarPosition(initiative: Initiative) {
    const startDays = Math.ceil((initiative.startDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
    const duration = Math.ceil((initiative.endDate.getTime() - initiative.startDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    >
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="px-4 py-2 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200/50">
          <span className="text-sm font-semibold text-gray-700">{initiatives.length} Initiatives</span>
        </div>
      </div>
