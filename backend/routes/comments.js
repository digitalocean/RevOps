...
async function canAccessItem(pool, userId, itemId) {
  const r = await pool.query('SELECT project_id FROM items WHERE id=$1', [itemId]);
...
router.get('/items/:itemId/comments', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT c.*, COALESCE(u.full_name, u.name) as author_name, u.email as author_email
...
router.post('/items/:itemId/comments', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO item_comments(item_id, author_id, body, mentions)
       VALUES($1,$2,$3,$4) RETURNING *`,
      [req.params.itemId, userId, body.trim(), mentions]
    );
    ...
    if (mentions && mentions.length > 0) {
      const itemRes = await pool.query('SELECT title, project_id FROM items WHERE id=$1', [req.params.itemId]);
      ...
      for (const mention of mentions) {
        const mentionedUser = await pool.query("SELECT id FROM users WHERE name ILIKE $1 LIMIT 1", [mention]).catch(() => ({ rows: [] }));
...
router.delete('/comments/:id', async (req, res) => {
  try {
    const userId = requireUser(req, res);
    if (!userId) return;
    const r = await pool.query('SELECT * FROM item_comments WHERE id=$1', [req.params.id]);
...
router.get('/items/:itemId/attachments', async (req, res) => {
  try {
    ...
    if (!ok) return res.status(404).json({ error: 'Not found' });
    const { rows } = await pool.query(
      `SELECT a.*, u.name as uploader_name FROM item_attachments a
       LEFT JOIN users u ON u.id = a.uploader_id
       WHERE a.item_id = $1 ORDER BY a.created_at ASC`,
      [req.params.itemId]
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
...
router.post('/items/:itemId/attachments', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO item_attachments(item_id, uploader_id, name, url, size_bytes, mime_type)
       VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,
      [req.params.itemId, userId, name, url, size_bytes, mime_type]
    );
    if (req.app.locals.broadcast) {
...
router.delete('/attachments/:id', async (req, res) => {
  try {
    const userId = requireUser(req, res);
    if (!userId) return;
    const r = await pool.query('SELECT * FROM item_attachments WHERE id=$1', [req.params.id]);
    if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
    if (String(r.rows[0].uploader_id) !== String(userId)) return res.status(403).json({ error: 'Forbidden' });
    await pool.query('DELETE FROM item_attachments WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
