...
async function canAccessTask(pool, userId, taskId) {
  const item = await pool.query('SELECT project_id FROM items WHERE id = $1', [taskId]);
...
router.get('/tasks/:id/field-values', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT fv.*, cf.name as field_name, cf.field_type, cf.field_key
...
router.patch('/tasks/:id/field-values', async (req, res) => {
  try {
    ...
    for (const v of values) {
      ...
      if (!fieldId) continue;
      const { rows } = await pool.query(
        `INSERT INTO custom_field_values (task_id, field_id, value_text, value_number, value_date, value_boolean, value_json, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
         ON CONFLICT (task_id, field_id) DO UPDATE SET
           value_text = COALESCE(EXCLUDED.value_text, custom_field_values.value_text),
           value_number = COALESCE(EXCLUDED.value_number, custom_field_values.value_number),
           value_date = COALESCE(EXCLUDED.value_date, custom_field_values.value_date),
           value_boolean = COALESCE(EXCLUDED.value_boolean, custom_field_values.value_boolean),
           value_json = COALESCE(EXCLUDED.value_json, custom_field_values.value_json),
           updated_at = NOW()
         RETURNING *`,
        [taskId, fieldId, valueText ?? null, valueNumber ?? null, valueDate ?? null, valueBoolean ?? null, valueJson ? JSON.stringify(valueJson) : null]
      );
