...
router.post('/', async (req,res)=>{ try{
  ...
  const {rows}=await pool.query(`INSERT INTO projects(workspace_id,name,description,color,owner_id) VALUES($1,$2,$3,$4,$5) RETURNING *`,[workspace_id,name,description,color,owner_id]);
  const pid = rows[0].id;
