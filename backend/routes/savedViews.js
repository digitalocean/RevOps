  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM saved_views
       WHERE project_id = $1 AND (user_id = $2 OR is_shared = true)
...
router.post('/projects/:id/views', async (req, res) => {
  try {
    ...
    if (is_default) {
      await pool.query(
        'UPDATE saved_views SET is_default = false WHERE project_id = $1 AND user_id = $2',
        [projectId, userId]
      );
    }
    const { rows } = await pool.query(
      `INSERT INTO saved_views (project_id, user_id, name, view_type, filters, column_config, sort_config, is_default, is_shared)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       ON CONFLICT (project_id, user_id, name) DO UPDATE SET
         ...
         updated_at = NOW()
       RETURNING *`,
      [projectId, userId, name.trim(), view_type, JSON.stringify(filters), JSON.stringify(column_config), JSON.stringify(sort_config), !!is_default, !!is_shared]
    );
...
router.patch('/views/:id', async (req, res) => {
  try {
    const userId = requireUser(req, res);
    if (!userId) return;
    const view = await pool.query('SELECT * FROM saved_views WHERE id = $1', [req.params.id]);
    ...
    if (is_default !== undefined) {
      ...
      if (is_default) {
        await pool.query(
          'UPDATE saved_views SET is_default = false WHERE project_id = $1 AND user_id = $2 AND id != $3',
          [view.rows[0].project_id, userId, req.params.id]
        );
      }
    }
    ...
    const { rows } = await pool.query(
      `UPDATE saved_views SET ${updates.join(', ')} WHERE id = $${i} RETURNING *`,
      values
    );
    res.json(rows[0]);
  } catch (e) {
