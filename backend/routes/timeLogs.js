...
router.get('/items/:itemId/time-logs', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT t.*, COALESCE(u.full_name, u.name) as user_name FROM time_logs t
       LEFT JOIN users u ON u.id = t.user_id
       WHERE t.item_id = $1 ORDER BY t.logged_at DESC`,
      [req.params.itemId]
    );
...
router.post('/items/:itemId/time-logs', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO time_logs(item_id, user_id, minutes, note, logged_at) VALUES($1,$2,$3,$4,$5) RETURNING *`,
      [req.params.itemId, userId, minutes, note || null, logged_at || new Date().toISOString().slice(0,10)]
    );
...
router.delete('/time-logs/:id', async (req, res) => {
  try {
    const userId = requireUser(req, res);
    if (!userId) return;
    const r = await pool.query('SELECT * FROM time_logs WHERE id=$1', [req.params.id]);
