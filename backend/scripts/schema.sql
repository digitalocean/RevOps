...
-- First / last name (share form + owner display)
DO $$ BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'crew' AND column_name = 'first_name') THEN
    ALTER TABLE crew ADD COLUMN first_name VARCHAR(80);
  END IF;
