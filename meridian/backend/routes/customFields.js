...
router.get('/', async (req,res)=>{ try{ const {workspace_id,target}=req.query; let q=`SELECT * FROM custom_fields WHERE workspace_id=$1`; const p=[workspace_id]; if(target){p.push(target);q+=` AND target=$${p.length}`;} q+=' ORDER BY sort_order,created_at'; const {rows}=await pool.query(q,p); res.json(rows); }catch(e){res.status(500).json({error:e.message});} });
